...
import { registerProvidedFactories, assignOpts } from "./helpers/factory";
import { Sywac, SywacOptions, Type, Factory, TypeOptions, SywacProvider, WithOption, SywacHandler } from "./_api";
...
export class Api<A = {}> {
  ...
  static get<A extends {} = {}>(opts?: SywacOptions) {
    return new Api<A>(opts) as unknown as Sywac<A>
  }
  static normalizeTypeOpts<O extends TypeOptions<any>>(flags: string | string[] | Partial<O>, opts: Partial<O>) {
    opts = opts || {}
  ...
  private _factories: Record<string, Factory<any, any, Sywac<A>>> = {}
  ...
  private constructor(opts?: SywacOptions) {
    opts = opts ?? {}
    ...
    this.configure(opts)
    if (!this.SYWAC.ROOT_NAME) this.SYWAC.ROOT_NAME = this.name
  }

  configure(opts?: SywacOptions) {
    opts = opts ?? {}
  ...
  newChild(commandName: string, childOptions: SywacOptions) {
  ...
  get unknownType() {
    if (!this._unknownType) this._unknownType = this.get<{}, Type<unknown>>('unknownType')!.withParent(Api.ROOT_NAME!)
  ...
  // type factories
  registerFactory<T, O extends {} = {}>(name: string, factory?: Factory<O, T, Sywac<A>>) {
    if (name && typeof factory === 'function') this._factories[name] = factory
    const f = factory as any as {SYWAC:SywacProvider};
    f.SYWAC = this.SYWAC
    return this
  }
  registerOption<N extends string, O extends TypeOptions<V>, V>(name: string, shorcut: N, factory?: Factory<O, Type<V>, Sywac<A>>) {
    const that = this as any as this & WithOption<N, Sywac<A>, O>;
    if (factory) this.registerFactory(name, factory);
    //@ts-ignore
    that[shorcut] = ((flags: string | O, opts?: O) => {
      return that._addOptionType(flags, opts, 'helpType') as any
    }) as any as this & WithOption<N, Sywac<A>, O>;
    return that
  }

  get<O extends {}, T>(name: string, opts?: O) {
    const that = this as Sywac<A>
    if (name && this._factories[name]) return (this._factories[name] as Factory<O, T, Sywac<A>>).call(that, opts!)
    return null
  }
  ...
  command(dsl: string, opts?: any /*TypeCommandOptions<A>*/) {
    this._internalCommand(dsl, opts)
    return this
  }

  private _internalCommand(dsl: string, opts?:  any/*TypeCommandOptions<A>*/| Function) {
    opts = opts || {} 
    // argument shuffling
    ...
    const commandType = this.get('commandType', opts) as any
    this.custom(commandType)
    return commandType
  }

  positional<T>(dsl: string|any , opts?: any) {
