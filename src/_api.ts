...
import { Sywac, SywacOptions, Type, Factory, TypeOptions, SywacProvider, WithOption, SywacHandler } from "./api";
...
class Api<A = {}> implements Sywac<A> {
  ...
  static get<A>(opts?: SywacOptions) {
  ...
  command(dsl: string, opts?: TypeCommandOptions<A>) {
    this._internalCommand(dsl, opts)
    return this
  }

  private _internalCommand(dsl: string, opts?: TypeCommandOptions<A> | Function) {
    opts = opts || {} as TypeCommandOptions<A>

    // argument shuffling
    if (typeof opts === 'function') {
      opts = { run: opts } as TypeCommandOptions<A>
    }
    ...
    const commandType = this.get('commandType', opts) as TypeCommand<A>
