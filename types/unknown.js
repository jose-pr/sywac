...
class TypeUnknown extends type_1.default {
    ...
    async parse(context) {
        ...
        unknownSlurped.forEach(arg => {
            arg.parsed.forEach(kv => {
                if (kv.key)
                    context.argv[kv.key] = kv.value; // TODO attempt to coerce to correct type?
                else
                    unparsed.push({ raw: arg.raw, index: arg.index });
            });
        ...
        if (unparsed.length) {
            ...
            if (implicitCommands.length)
                unparsed = this._tryImplicitCommand(unparsed, context, implicitCommands);
        }
    ...
    _populatePositionals(unparsed, context) {
        ...
        unparsed.forEach(arg => {
            if (!current)
                return; // more args than positionals
