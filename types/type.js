...
class Type {
    ...
    configure(opts, override = true) {
        ...
        if (override || typeof this._defaultVal === 'undefined')
            this._defaultVal = 'defaultValue' in opts ? opts.defaultValue : this._defaultVal;
        if (override || typeof this._required === 'undefined')
            this._required = 'required' in opts ? opts.required : this._required;
        if (override || typeof this._strict === 'undefined')
            this._strict = 'strict' in opts ? opts.strict : this._strict;
        if (override || typeof this._coerceHandler !== 'function')
            this._coerceHandler = opts.coerce || this._coerceHandler;
        // configurable for help text
        if (override || !this._flags)
            this._flags = opts.flags || this._flags;
        if (override || !this._desc)
            this._desc = opts.description || opts.desc || this._desc;
        if (override || typeof this._hints === 'undefined')
            this._hints = 'hints' in opts ? opts.hints : this._hints;
        if (override || !this._group)
            this._group = opts.group || this._group;
        if (override || typeof this._hidden === 'undefined')
    ...
    alias(a) {
        if (a)
            this._aliases = this._aliases.concat(a);
        return this;
    }
    get aliases() {
    ...
    get helpHints() {
        if (typeof this._hints !== 'undefined')
            return this._hints;
        const hints = [];
    ...
    buildHelpHints(hintsArray) {
        // required
        if (this.isRequired)
            hintsArray.push('required');
        // datatype
        if (this.datatype)
            hintsArray.push(this.datatype);
        // default value
        const dv = this._defaultVal;
        if (dv && (!Array.isArray(dv) || dv.length))
            hintsArray.push(`default: ${dv}`);
    }
    group(g) {
        this._group = g;
        return this;
