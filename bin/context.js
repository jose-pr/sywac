...
class Context {
    ...
    slurpArgs(args) {
        if (typeof args === 'string')
            args = this.utils.stringToArgs(args);
        if (!args)
    ...
    parseSingleArg(arg) {
        ...
        if (value)
            kvArray[kvArray.length - 1].value = value;
        return kvArray;
    }
    ...
    matchCommand(level, aliases, isDefault) {
        if (!this.argv._ || this.versionRequested)
            return false; // TODO what to do without an unknownType?
    ...
    addDeferredVersion() {
        ...
        if (typeof this.versionRequested.version === 'function')
            this.output = this.versionRequested.version();
        else
            this.output = this.versionRequested.version;
        return this;
    }
    ...
    populateArgv(typeResults) {
        ...
        typeResults.forEach(tr => {
            ...
            if (detailIndex !== -1)
                this.details.types[detailIndex] = tr;
            else
                this.details.types.push(tr);
            // if not command, set value for each alias in argv
            if (tr.datatype === 'command')
                return undefined; // do not add command aliases to argv
